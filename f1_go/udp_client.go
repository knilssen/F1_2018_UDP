...
type LapData struct {
    ...
    M_lapDistance float32                  // Distance vehicle is around current lap in metres – could
                                         // be negative if line hasn’t been crossed yet
    M_totalDistance                      float32                // Total distance travelled in session in metres – could
                                         // be negative if line hasn’t been crossed yet
...
type ParticipantData struct {
    ...
    M_name[48]  string                      // Name of participant in UTF-8 format – null terminated
...
type PacketCarTelemetryData struct {
    ...
    M_buttonStatus  uint32                      // Bit flags specifying which buttons are being
...
type CarStatusData struct {
    ...
    M_tyreCompound  uint8                   // Modern - 0 = hyper soft, 1 = ultra soft
    ...
    M_ersDeployMode uint8                   // ERS deployment mode, 0 = none, 1 = low, 2 = medium
...
func main() {
  ...
  var motion_packet PacketMotionData
  var session_packet PacketSessionData
  var lap_packet PacketLapData
  var event_packet PacketEventData
  var participant_packet PacketParticipantsData
  var car_setup_packet PacketCarSetupData
  var telemetry_packet PacketCarTelemetryData
  var car_status_packet PacketCarStatusData
